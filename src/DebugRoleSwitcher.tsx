...
export const DebugRoleSwitcher: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      {isOpen ? (
        <div className="bg-white border border-gray-200 shadow-lg rounded-lg p-3 w-56 text-xs">
            ...
            {users.map(u => (
              <button
                ...
                className={`w-full text-left px-2 py-1.5 rounded transition-colors ${
                  user?.id === u.id
                    ? 'bg-brand bg-opacity-10 text-brand font-semibold'
        ...
        <button
          ...
          className="bg-brand text-white pl-3 pr-4 h-9 rounded-full shadow-md hover:bg-brand-hover flex items-center gap-2 transition-colors focus:outline-none text-xs font-semibold"
          title="Preview As…"
        >
