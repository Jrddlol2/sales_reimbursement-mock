    ...
    <ConfirmContext.Provider value={confirm}>
      ...
      {options && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900 bg-opacity-40" onClick={() => handle(false)} />
          <div className="relative bg-white rounded-lg shadow-2xl max-w-md w-full border border-gray-200">
            <div className={`p-5 border-b ${isDanger ? 'bg-red-50 border-red-100' : 'bg-amber-50 border-amber-100'} rounded-t-lg`}>
              <div className="flex items-start gap-3">
                <AlertTriangle className={`w-5 h-5 shrink-0 mt-0.5 ${isDanger ? 'text-red-500' : 'text-amber-500'}`} />
                <div>
            ...
            <div className="p-4 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-1.5 border border-gray-300 text-gray-700 rounded text-sm font-medium hover:bg-gray-50"
              >
                {options.cancelLabel || 'Cancel'}
              </button>
              <button
                onClick={() => handle(true)}
                className={`px-4 py-1.5 text-white rounded text-sm font-semibold shadow-sm ${
