...
export const KPICard: React.FC<KPICardProps> = ({ 
  ...
  const variantStyles = {
    action: {
      card: 'bg-white border-2 border-indigo-500 shadow-sm hover:shadow-md transition-shadow',
    ...
    success: {
      card: 'bg-white border border-emerald-200 hover:border-emerald-300 transition-colors',
    ...
    warning: {
      card: 'bg-white border border-amber-200 hover:border-amber-300 transition-colors',
      iconContainer: 'bg-amber-50 text-amber-600',
      title: 'text-slate-600 font-medium',
      value: 'text-amber-700',
      description: 'text-slate-500',
      context: 'text-amber-600/70',
      btn: 'bg-amber-50 text-amber-700 hover:bg-amber-100',
      topBar: 'bg-amber-400'
    },
    info: {
      card: 'bg-white border border-slate-200 hover:border-slate-300 transition-colors',
    ...
    default: {
      card: `bg-white border border-slate-200 hover:border-brand/40 transition-colors ${colorClass}`,
      iconContainer: 'bg-brand/10 text-brand',
      title: 'text-slate-500 font-semibold',
      value: 'text-brand',
      description: 'text-slate-500',
      context: 'text-slate-400',
      btn: 'bg-slate-100 text-slate-700 hover:bg-slate-200',
    ...
    <>
      ...
      <div className="flex items-start justify-between mb-4 mt-1">
        <div className="flex gap-3 items-center">
          ...
          <div>
            <h3 className={`text-xs uppercase tracking-wider leading-normal ${styles.title}`}>{title}</h3>
            {description && <p className={`text-[11px] mt-0.5 leading-normal ${styles.description}`}>{description}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="flex items-end justify-between">
        <div className="flex flex-col">
          <div className={`text-3xl font-extrabold tracking-tight ${styles.value}`}>
            {value}
          </div>
          {additionalContext && (
            <div className={`text-[10px] mt-1 leading-normal ${styles.context}`}>
              {additionalContext}
            </div>
          )}
        </div>
