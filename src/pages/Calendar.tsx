...
export const Calendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    apiFetch('/api/activity/seen', {
      method: 'POST',
      body: JSON.stringify({ section: 'calendar' })
    }).catch(console.error);
  }, []);
  ...
  const fetchReviewMeetings = async () => {
    try {
      const data = await apiFetch('/api/review-meetings');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-950 tracking-tight font-display">Calendar</h2>
      ...
      <CalendarGrid moms={moms} reviewMeetings={reviewMeetings} />
    </div>
  );
};
